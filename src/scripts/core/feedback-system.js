...
function getPerformanceModeStatus() {
  try {
    return localStorage.getItem("broadcast-generator-performance-mode") ===
      "true"
      ? "On"
      : "Off";
  } catch {
...
function getAudioMutedStatus() {
  try {
    return localStorage.getItem("broadcast-generator-audio-muted") === "true"
      ? "Off"
      : "On";
  } catch {
    return "Unknown";
...
function getSystemInfo() {
  ...
  const info = {
    ...
    pageLoadTime: timing
      ? Math.max(0, Math.round(timing.loadEventEnd - timing.navigationStart)) +
        "ms"
      : "N/A",
...
function formatSystemInfoMarkdown(info) {
  const lines = [
    ...
    info.network
      ? `• Network: ${info.network.type} | ↓ ${info.network.downlink}Mb/s | rtt ${info.network.rtt}ms | saveData ${info.network.saveData}`
      : null,
...
function typeIconSvg(type) {
  ...
  switch (type) {
    case "bug":
      return `<svg ${common}><path d="M8 9v-3a4 4 0 0 1 8 0v3"/><path d="M3 13h18"/><path d="M19 7l-3 2"/><path d="M5 7l3 2"/><rect x="8" y="9" width="8" height="10" rx="4"/></svg>`;
    case "feature":
      return `<svg ${common}><path d="M12 2l2.39 7.34H22l-6.19 4.5L18.2 21 12 16.9 5.8 21l2.39-7.16L2 9.34h7.61z"/></svg>`;
    case "suggestion":
      return `<svg ${common}><circle cx="12" cy="12" r="4"/><path d="M2 12h4"/><path d="M18 12h4"/><path d="M12 2v4"/><path d="M12 18v4"/></svg>`;
    case "improvement":
      return `<svg ${common}><path d="M12 19V5"/><path d="M5 12l7-7 7 7"/></svg>`;
    default:
      return `<svg ${common}><path d="M4 19.5V4.5a2 2 0 0 1 2-2h8l6 6v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2z"/></svg>`;
  }
}
...
function fieldIconSvg(field) {
  ...
  switch (field) {
    ...
    case "repro":
      return `<svg ${common}><path d="M21.5 2v6h-6"/><path d="M3 12.5a9 9 0 0 1 15-8.5"/><path d="M2.5 22v-6h6"/><path d="M21 11.5a9 9 0 0 1-15 8.5"/></svg>`;
    case "categories":
    ...
    case "steps":
      return `<svg ${common}><path d="M12 5v14"/><path d="M19 12l-7 7-7-7"/><circle cx="12" cy="5" r="1"/></svg>`;
    case "expected":
      return `<svg ${common}><polyline points="20 6 9 17 4 12"/></svg>`;
    case "actual":
      return `<svg ${common}><circle cx="12" cy="12" r="10"/><path d="M12 6v6m0 0v6"/></svg>`;
    case "logs":
      return `<svg ${common}><path d="M4 7h16M4 12h16M4 17h16"/><path d="M4 3v18"/></svg>`;
    case "system":
      return `<svg ${common}><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M2 17h20"/></svg>`;
    default:
      return `<svg ${common}><path d="M6 9l6-6 6 6m0 8l-6 6-6-6"/></svg>`;
  }
}
...
function createFeedbackUI() {
  ...
  if (
    !window.turnstile &&
    !document.querySelector(`script[src="${TURNSTILE_SCRIPT}"]`)
  ) {
...
function getCurrentPage() {
  ...
  if (path.includes("broadcast") && path.includes("simple"))
    return "broadcast-simple";
  if (path.includes("broadcast") && path.includes("advanced"))
    return "broadcast-advanced";
...
function updateFieldVisibility(type) {
  ...
  const bugFeatureOtherImpFields = document.querySelectorAll(
...
function updateDataInfo(type) {
  ...
  switch (type) {
    case "bug":
      infoText =
        "This report will include: Console logs + System info + Performance status + Audio status";
      break;
    case "feature":
    ...
    case "suggestion":
      infoText =
        "This report will include: System info + Performance status + Audio status";
      break;
    case "improvement":
      infoText =
        "This report will include: Console logs + Performance status + Audio status";
      break;
    case "other":
      infoText = "";
      break;
...
function setupFeedbackListeners() {
  ...
  pageSelect.innerHTML = PAGES.map(
...
async function submitFeedback(form) {
  ...
  try {
    ...
    let turnstileContainer = document.getElementById(
      "hiddenTurnstileContainer",
    );
    ...
    const turnstileToken = await new Promise((resolve, reject) => {
      ...
      try {
        ...
        widgetId = window.turnstile.render("#hiddenTurnstileContainer", {
          ...
          callback: (token) => {
            console.log(
              "[Turnstile] Callback fired with token:",
              token ? "YES" : "NO",
            );
            if (!resolved && token) {
              resolved = true;
          ...
          const pollInterval = setInterval(() => {
            if (!resolved && widgetId !== null) {
              try {
                ...
                if (pollCount % 10 === 0) {
                  console.log(
                    `[Turnstile] Poll attempt ${pollCount}, token: ${token ? "YES" : "NO"}`,
                  );
                }
                if (token) {
    ...
    const sysInfoBlock = includeSystemInfo
      ? formatSystemInfoMarkdown(getSystemInfo())
      : "";
    const loggerBuf = getLogBuffer();
    const logsText =
      includeLogs && loggerBuf.length > 0
        ? loggerBuf.slice(-80).join("\n")
        : "";
    ...
    const color =
      FEEDBACK_TYPES.find((t) => t.value === feedbackType)?.color || 0xff3333;
    const embed = {
      ...
      fields: [
        {
          name: "Page",
          value: String(formData.get("page") || "Unknown"),
          inline: true,
        },
    ...
    if (includePerfAudio && perfStatus)
      embed.fields.push({
        name: "Performance Status",
        value: perfStatus,
        inline: true,
      });
    if (includePerfAudio && audioStatus)
      embed.fields.push({ name: "Audio", value: audioStatus, inline: true });
    if (categories)
      embed.fields.push({
        name: "Categories",
        value: categories.split(",").join(", ").slice(0, 500),
        inline: true,
      });
    embed.fields.push({ name: "Reproducibility", value: repro, inline: true });
    if (steps)
      embed.fields.push({ name: "Steps", value: steps.slice(0, 1024) });
    if (expected)
      embed.fields.push({ name: "Expected", value: expected.slice(0, 1024) });
    if (actual)
      embed.fields.push({ name: "Actual", value: actual.slice(0, 1024) });
    if (sysInfoBlock)
      embed.fields.push({
        name: "System",
    ...
    console.log(
      "[Feedback] Sending feedback with token:",
      turnstileToken ? "YES" : "NO",
    );
    console.log(
      "[Feedback] Token preview:",
    ...
    if (!res.ok) {
      ...
      try {
        ...
        if (
          errorJson.error === "Captcha token required" ||
          errorJson.message?.includes("captcha")
        ) {
          errorMessage =
    ...
    if (labelEl) {
      ...
      labelEl.textContent =
        errorMsg.length > 30 ? errorMsg.substring(0, 27) + "..." : errorMsg;
...
async function uploadScreenshotToRepo(token, owner, repo, blob) {
  ...
  return (
    (data.content && (data.content.download_url || data.content.html_url)) || ""
  );
}
