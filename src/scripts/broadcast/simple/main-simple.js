    ...
    allLists.forEach((l) => {
      if (l !== list) {
        ...
        const b = document.querySelector(
          `.menu-btn[data-menu="${l.dataset.menu}"]`,
        );
...
document.querySelectorAll(".menu-list button").forEach((optBtn) => {
  optBtn.addEventListener("click", () => {
    ...
    const menuBtn = document.querySelector(
      `.menu-btn[data-menu="${parentMenu.id}"]`,
    );
...
document.getElementById("generateBtn")?.addEventListener("click", () => {
  ...
  const statusMessages = {
    ...
    "SITE LOCKDOWN":
      "Site lockdown initiated armed teams are to respond and await further instructions",
    "CLASS-D ESCAPE":
      "Class-D personnel have escaped containment. All staff stay alert",
    "CHAOS INSURGENCY":
  ...
  const eventMessages = {
    ...
    "323 BREACH":
      "SCP-323 containment All personnel evacuate immediately. Response teams engage with full-force authorization.",
  };
  ...
  if (
    ...
    (status === "SCP BREACH" &&
      (eventSelected === "076 EVENT" ||
        eventSelected === "610 EVENT" ||
        eventSelected === "323 BREACH")) ||
    // Regla 2: CLASS-D ESCAPE + CLASS-D RIOT → solo evento
    (status === "CLASS-D ESCAPE" && eventSelected === "CLASS-D RIOT") ||
